...
{
    public class AdvancedPage(IServiceProvider services, ResourcesService resourcesService, ITelegramService telegramService, ITelegramBotClient client) : MessagePhotoPageBase(resourcesService, telegramService)
    {
        ...
        public override UserState ProcessMessageAsync(Message message, UserState userState)
        {
            try
            {
                ...
                var course = FeedbackStorage.Tutors["AdvancedPage"];
            ...
            {
                Log.Error(ex, $"Ошибка {ex} на странице AdvancedPage");
                userState.requestCounter = 0;
                return userState;
            }
        }
            ...
            try
            {
                if (userName == string.Empty)
                {
                    await client.SendTextMessageAsync(
                        ...
                        text: $"Студент {userFirstName} просит в курсе Для продвинутых ответить на следующий вопрос:{Environment.NewLine}{Environment.NewLine}{userMessage}",
                        parseMode: ParseMode.MarkdownV2);
                }
                else
                    await client.SendTextMessageAsync(
                        chatId: managerChatId,
                        $"Пользователь [{userFirstName}](http://t\\.me/{userName}) в курсе Для продвинутых прислал сообщение:{Environment.NewLine}{Environment.NewLine}{userMessage}",
                        parseMode: ParseMode.MarkdownV2);
            }
            catch (Exception ex)
            {
                Log.Error(ex, $"Ошибка {ex} на странице AdvancedPage в методе SendMessageRequestAsync");
            }
        }
