        ...
        private async Task SendMessageRequestAsync(long managerChatId, string? userName, string? userFirstName, string? userMessage)
        {
            if (userName == string.Empty)
            {
                await client.SendTextMessageAsync(
                    ...
                    text: $"Студент {userFirstName} просит в курсе Для продвинутых ответить на следующий вопрос:{Environment.NewLine}{userMessage}",
                    parseMode: ParseMode.MarkdownV2);
            }
            else
                await client.SendTextMessageAsync(
                    chatId: managerChatId,
                    $"Пользователь [{userFirstName}](http://t\\.me/{userName}) в курсе Для продвинутых прислал сообщение{Environment.NewLine}{userMessage}",
                    parseMode: ParseMode.MarkdownV2);
        }
