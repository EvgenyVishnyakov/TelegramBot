...
{
    public abstract class MessagePhotoPasswordPageBase(ResourcesService resourcesService, ITelegramService telegramService) : CallbackQueryPhotoPageBase(resourcesService, telegramService)
    {
        ...
        public override PageResultBase Handle(Update update, UserState userState)
        {
            try
            {
                ...
                if (!updateUserState.IsPassword)
                {
                    var nextPage = GetStartPage();
