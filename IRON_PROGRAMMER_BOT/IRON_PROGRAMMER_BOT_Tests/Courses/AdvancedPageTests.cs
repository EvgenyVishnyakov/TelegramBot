...
namespace IRON_PROGRAMMER_BOT_Tests.Courses
{
    public class AdvancedPageTests
    {
        ...
        {
            //Arrange
            var advancedPage = _services.GetRequiredService<AdvancedPage>();
            ...
            //Act
            var result = advancedPage.View(null, userState);

            //Assert
            ClassicAssert.IsInstanceOf<PhotoPageResult>(result);

            Assert.That(result.UpdatedUserState.CurrentPage, Is.EqualTo(advancedPage));
        ...
        public void Handle_ConnectWithTutorPageCallback_ConnectWithTutorPage()
        {
            //Arrange
            var advancedPage = _services.GetRequiredService<AdvancedPage>();
            var pages = new Stack<IPage>([_services.GetRequiredService<NotStatedPage>(), _services.GetRequiredService<StartPage>(), _services.GetRequiredService<ConnectWithTutorPage>(), advancedPage]);
            var userState = new UserState(pages, new UserData());
            var update = new Update() { CallbackQuery = new CallbackQuery() { Data = Resources.Back } };

            //Act
            var result = advancedPage.Handle(update, userState);
        ...
        [Test]
        public void Handle_UnknownMessage_AdvancedPage()
        {
            //Arrange
            var advancedPage = _services.GetRequiredService<AdvancedPage>();
            var pages = new Stack<IPage>([_services.GetRequiredService<NotStatedPage>(), _services.GetRequiredService<StartPage>(), _services.GetRequiredService<ConnectWithTutorPage>()]);
            ...
            //Act
            var result = advancedPage.View(update, userState);
