...
namespace IRON_PROGRAMMER_BOT_Tests.Courses
{
    public class AlgorithmPageTests
    {
        ...
        {
            //Arrange
            var algorithmPage = _services.GetRequiredService<AlgorithmPage>();
            ...
            //Act
            var result = algorithmPage.View(null, userState);

            //Assert
            ClassicAssert.IsInstanceOf<PhotoPageResult>(result);

            Assert.That(result.UpdatedUserState.CurrentPage, Is.EqualTo(algorithmPage));
        ...
        public void Handle_ConnectWithTutorPageCallback_ConnectWithTutorPage()
        {
            //Arrange
            var algorithmPage = _services.GetRequiredService<AlgorithmPage>();
            var pages = new Stack<IPage>([_services.GetRequiredService<NotStatedPage>(), _services.GetRequiredService<StartPage>(), _services.GetRequiredService<ConnectWithTutorPage>(), algorithmPage]);
            var userState = new UserState(pages, new UserData());
            var update = new Update() { CallbackQuery = new CallbackQuery() { Data = Resources.Back } };

            //Act
            var result = algorithmPage.Handle(update, userState);
        ...
        [Test]
        public void Handle_UnknownMessage_AlgorithmPage()
        {
            //Arrange
            var algorithmPage = _services.GetRequiredService<AlgorithmPage>();
            var pages = new Stack<IPage>([_services.GetRequiredService<NotStatedPage>(), _services.GetRequiredService<StartPage>(), _services.GetRequiredService<ConnectWithTutorPage>()]);
            ...
            //Act
            var result = algorithmPage.View(update, userState);
