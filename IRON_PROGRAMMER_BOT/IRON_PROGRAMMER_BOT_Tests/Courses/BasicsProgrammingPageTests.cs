...
{
    public class BasicsProgrammingPageTests
    {
        ...
        {
            //Arrange
            var basicsProgrammingPage = _services.GetRequiredService<BasicsProgrammingPage>();
            ...
            //Act
            var result = basicsProgrammingPage.View(null, userState);

            //Assert
            ClassicAssert.IsInstanceOf<PhotoPageResult>(result);

            Assert.That(result.UpdatedUserState.CurrentPage, Is.EqualTo(basicsProgrammingPage));
        ...
        public void Handle_ConnectWithTutorPageCallback_ConnectWithTutorPage()
        {
            //Arrange
            var basicsProgrammingPage = _services.GetRequiredService<BasicsProgrammingPage>();
            var pages = new Stack<IPage>([_services.GetRequiredService<NotStatedPage>(), _services.GetRequiredService<StartPage>(), _services.GetRequiredService<ConnectWithTutorPage>(), basicsProgrammingPage]);
            var userState = new UserState(pages, new UserData());
            var update = new Update() { CallbackQuery = new CallbackQuery() { Data = Resources.Back } };

            //Act
            var result = basicsProgrammingPage.Handle(update, userState);
        ...
        [Test]
        public void Handle_UnknownMessage_BasicsProgrammingPage()
        {
            //Arrange
            var basicsProgrammingPage = _services.GetRequiredService<BasicsProgrammingPage>();
            var pages = new Stack<IPage>([_services.GetRequiredService<NotStatedPage>(), _services.GetRequiredService<StartPage>(), _services.GetRequiredService<ConnectWithTutorPage>()]);
            ...
            //Act
            var result = basicsProgrammingPage.View(update, userState);
